...
class Button(discord.ui.Button['Gobblers']):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if state.kind == player.kind:
            await interaction.response.send_message(embed=discord.Embed(description='You already have this piece', color=discord.Color.dark_grey()), ephemeral=True)
            return

        if player.available_strength <= state.strength:
            await interaction.response.send_message(
                embed=discord.Embed(description='You do not have the strength necessary to take down this piece', color=discord.Color.dark_grey()),
                ephemeral=True
            )
            return
        ...
        if player.current_selection is not None:
            await interaction.response.send_message(
                embed=discord.Embed(description="You've already selected a piece, you can't select multiple pieces.", color=discord.Color.dark_grey()),
                ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(embed=discord.Embed(description='Select a piece strength', color=discord.Color.dark_grey()), view=prompt)
        await prompt.wait()
        ...
        await interaction.message.edit(embed=discord.Embed(description=content, color=discord.Color.dark_grey()), view=self.view)
...
class Gobblers(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user not in (self.players[0].member, self.players[1].member):
            await interaction.response.send_message(embed=discord.Embed(description='This is a game between two other people, sorry.', color=discord.Color.dark_grey()), ephemeral=True)
            return False

        if interaction.user != self.current_player.member:
            await interaction.response.send_message(embed=discord.Embed(description="It's not your turn", color=discord.Color.dark_grey()), ephemeral=True)
            return False
...
class Prompt(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.second:
            await interaction.response.send_message(embed=discord.Embed(description='This prompt is not meant for you', color=discord.Color.dark_grey()), ephemeral=True)
            return False
        return True
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(
            embed=discord.Embed(description=f'Challenge accepted! {order[0].mention} goes first and {order[1].mention} goes second.\n\n'
            f"It is now \N{LARGE BLUE SQUARE} {order[0].mention}'s turn", color=discord.Color.dark_grey()),
    ...
    async def decline(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(embed=discord.Embed(description='Challenge declined :(', color=discord.Color.dark_grey()))
        self.stop()

        
class Goblet(commands.Cog):
    ...
    async def goblet(self, ctx: commands.Context, opponent: discord.Member):
        ...
        await ctx.send(f"{opponent.mention}, {ctx.author.mention} has challenged you to a game of Gobblet Gobblers!", view=prompt)
        await prompt.wait()

        if not prompt.confirmed:
            return


async def setup(bot):
    await bot.add_cog(Goblet(bot))
